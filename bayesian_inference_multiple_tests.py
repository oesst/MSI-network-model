...
exp_name = 'Bayesian_Inference_Cortical_Intensity_test'
if no_spatial_conv:
    exp_name += '_Dirac_Kernels'


s_a_mean_list = [9 ,10, 10.5, 11, 11.5, 12, 12.5]
s_v_mean_list = [8]
s_a_var_list = [1.5]
s_v_var_list = [1.0]
s_a_intensity_list = [0.1,0.25,0.5,0.75,0.9]
s_v_intensity_list = [0.5]

n_draws = 100

list_all = [(a, b, c, d, e, f)
            for a in s_a_mean_list for b in s_v_mean_list for c in s_a_var_list for d in s_v_var_list for e in s_a_intensity_list for f in s_v_intensity_list]

for s_a_mean, s_v_mean, s_a_var, s_v_var, s_a_intensity, s_v_intensity in list_all:

    # walk over all different parameter combinations
    ...
    if os.path.exists(exp_dir) and os.listdir(exp_dir):
        ...
        # Python 3: open(..., 'wb')
        with open(os.path.join(exp_dir, exp_name_neurons + '.txt',), 'w+') as f:
            f.write(exp_name + '\n \n')
        ...
        for i_draw in range(n_draws):
            ...
            for i_condi, condi in enumerate(conditions_to_test):
                ...
                stimuli_s_a = {'loc': np.array([s_a_location]), 'onset': s_onset_temp,
                               'duration': net.len_t - s_onset_temp, 'sigma': sigma_s_a, 'weight': s_v_intensity}

                # cortical inputs
                stimuli_c_v = {'loc': np.array([s_v_location]), 'onset': s_onset_temp,
                               'duration': net.len_t - s_onset_temp, 'sigma': sigma_c_v, 'weight': s_a_intensity}
